...
public class TeamService {
    ...
    public Team get(UUID id) { return teamRepository.findById(id).orElseThrow(() -> new RuntimeException("Team not found")); }
    ...
    public void delete(UUID id) { teamRepository.deleteById(id); }
    ...
    public void removeMember(UUID id) { memberRepository.deleteById(id); }

    public List<TeamMember> membersForTeam(UUID teamId) { return memberRepository.findByTeamId(teamId); }

    public List<TeamMember> teamsForUser(UUID userId) { return memberRepository.findByUserId(userId); }
}
