...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Health Check", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Ping", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Get My Profile", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Update Profile", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Get All Users", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 201) { Write-Host "PASS - 201" -ForegroundColor Green; $results += @(@("Create Holiday", "PASS", 201)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("List Holidays", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 201) { Write-Host "PASS - 201" -ForegroundColor Green; $results += @(@("Request Leave", "PASS", 201)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Get My Leaves", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 201) { Write-Host "PASS - 201" -ForegroundColor Green; $results += @(@("Create Team", "PASS", 201)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("List Teams", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("My Sessions", "PASS", 200)) }
...
try {
    ...
    if ($resp.StatusCode -eq 200) { Write-Host "PASS - 200" -ForegroundColor Green; $results += @(@("Admin Attendance All", "PASS", 200)) }
...
try {
    ...
    Write-Host "FAIL - Expected 403 but got $($resp.StatusCode)" -ForegroundColor Red; $results += @(@("Unauthorized Access", "FAIL", $resp.StatusCode))
} catch {
...
if ($failed -gt 0) {
    ...
    $results | Where-Object { $_[1] -ne "PASS" } | ForEach-Object {
        Write-Host "  - $($_[0]): Status $($_[2])" -ForegroundColor Red
    }
}
